...
benchmark_data['threads'] = benchmark_data['name'].str.extract('Gol/(\d+)-\d+').apply(pd.to_numeric)
benchmark_data['cpu_cores'] = benchmark_data['name'].str.extract('Gol/\d+-(\d+)').apply(pd.to_numeric)
